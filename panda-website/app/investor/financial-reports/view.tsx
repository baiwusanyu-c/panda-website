...
import {FinancialReportsInfo} from "@/app/investor/financial-reports/api/route";
import { UpCircleOutlined, DownCircleOutlined } from '@ant-design/icons'
import {useState} from "react";

export default function FinancialReportsView(props: { list: FinancialReportsInfo['list']}) {
  const { list } = props;
    ...
    <div className="panda-tea-about overflow-x-hidden bg-cbd-white w-full">
      <motion.div
        ...
        <div className='fsc h-[570px] mx-auto -0 max-w-[1500px] min-w-[1200px] w-[94%] my-[120px]'>
            ...
            <div className='fcc flex-col w-full'>
              {
                list.map((item, index) => (
                  <div className='fcc flex-col w-full' key={item.title}>
                    ...
                    {
                      index !== list.length - 1 ? <span className='bg-cbd-gray-4 w-full h-[1px]'></span> : <></>
                    }
                  </div>
                ))
              }
          ...
          <a className='fcc flex-col ml-[120px]'
            ...
            <div className={`bg-cbd-brand-5 transition-all rounded-full h-[6px] mt-[20px] ${isHover ? 'w-1/2' : 'w-[0px]'}`}>
            </div>
          </a>
        </div>
      </motion.div>
