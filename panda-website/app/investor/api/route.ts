...
import {AnnouncementsNoticesResInfo} from "@/app/investor/announcements-notices/api/route";
...
async function getAnnouncementsNoticesData(){
  const res = await fetch('http://localhost:3000/investor/announcements-notices/api', { method: 'post' });
  return await res.json()
}
...
export async function POST() {
  ...
  const announcementsNoticesRes: { data: AnnouncementsNoticesResInfo } = await getAnnouncementsNoticesData()
  const data = {
    description: '自成立以来，茬白稻专注探索天然食材与中式茶饮的搭配，持续研发多元化茶饮产品，致力于提升消费者体验。我们通过互惠互利的加盟模式，现已经发展成为全国知名的现制茶饮企业。',
